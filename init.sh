...
mysql_init () {
	...
	#touch $CONF_PATH/my.cnf
	/usr/bin/mysql_install_db --defaults-file=/dev/null --datadir=$MYSQL_DATA_PATH --user=user_mysql
	/usr/sbin/mysqld --defaults-file=/dev/null --basedir=/usr --datadir=$MYSQL_DATA_PATH --log-error=/dev/null --user=user_mysql --pid-file=$MYSQL_DATA_PATH/mysqld.pid --socket=$MYSQL_DATA_PATH/mysqld.sock --skip-networking --plugin-dir=/usr/lib/mysql/plugin &
	sleep 5;
	/usr/bin/mysqladmin -S $MYSQL_DATA_PATH/mysqld.sock -u root password $MYSQL_PASSWORD
...
check_dir () {
	...
	[[ ! -d $WWW_DATA_PATH ]] && mkdir $WWW_DATA_PATH && chown user_web:user_web $WWW_DATA_PATH
}
